"""OCR extraction using OpenAI GPT-4o (vision)."""
import base64
import json
...
def extract_from_image(image_path: str) -> BoardingPass:
    """Extract boarding pass data from image using GPT-4o vision."""
    try:
        ...
        return BoardingPass(
            ocr_confidence=0.0,
            assumptions=[f"API Error: {str(e)}"]
        )


def extract_from_text(text: str) -> BoardingPass:
    """Fallback: extract from manual text input using OpenAI."""
    try:
        response = client.chat.completions.create(
            model=OPENAI_MODEL,
            messages=[
        ...
        return BoardingPass(
            ocr_confidence=0.0,
            assumptions=[f"Text parsing failed: {str(e)}"]
        )
